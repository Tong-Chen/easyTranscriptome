...
def main():
    ...
    print("\t".join(labelL), file=fh_out)
    keyL = list(aDict.keys())
    keyL.sort()
    key2L = list(bDict.keys())
    key2L.sort()
    resultL = []
    pL = []
    alreadyCntD = {}
    for key1 in keyL:
        ...
        for key2 in key2L:
            if key1 == key2:
                continue
            if pairD and (key1, key2) not in pairD:
                continue
            if (key2, key1) in alreadyCntD or (key1, key2) in alreadyCntD:
                continue
            alreadyCntD[(key1, key2)] = 1
            value2 = bDict[key2]
            if debug:
                print(key1, value1, file=sys.stderr)
                print(key2, value2, file=sys.stderr)
    ...
    print('\n'.join(['\t'.join(tmpL) for tmpL in resultL]), file=fh_out)
    if file != '-':
        fh.close()
    fh_out.close()
