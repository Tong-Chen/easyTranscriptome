...
install_bioc <-
  c(
    "ggExtra", "ggpur", "ggstatsplot", "Hmisc", "tidyr","ggcorrplot"
  )
...
```{r}
top6_de_gene <- de_gene %>% dplyr::group_by(V2) %>% dplyr::slice(1:3)
...
library(ggplot2)
ggplot(top6_t_with_group_melt, aes(x=Gene, y=Expr)) + 
  geom_boxplot(aes(fill=conditions)) + theme_classic() +
...
ggplot(top6_t_with_group_melt, aes(x=Gene, y=Expr)) + 
  geom_boxplot(aes(fill=conditions)) + theme_classic() +
  scale_fill_manual(values=ImageGP::generate_color_list("Set2",2))
```
...
```{r}
ImageGP::generate_color_list("Set2",12)
```
